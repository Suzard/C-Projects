...
void initializing_pre_reqs(struct prereq_class *class_data, char* offering_word, int class_counter){
	int iterating = 0;
//	printf("this is what's being offered %s\n", offering_word);
		...
		if(strcmp(class_data[iterating].name, offering_word) == 0){
			...
			while(1){
				...
				if(offering_word ==NULL )
					break;
//				printf("prereqs being oopied into %s\n", offering_word);
...
main(int argc, char *argv[])
{
	...
	printf("Processing... May take longer for larger files\n");	
	...
	int w;
	int s;
	for(f = 0; f <fall_counter;f++){
		printf("fall %s\n", fall_data[f].name);
	}
	for(w= 0;w <winter_counter;w++){

		printf("winter %s\n", winter_data[w].name);
	}
	for(s = 0;s<spring_counter;s++){
	...
	fclose(prereq_file);
	printf("cchecking inside %d\n", check_inside(fall_data,"b",fall_counter));
	...
	int zuper = 0;
	for(zuper = 0; zuper < recursion_counter; zuper++){
		printf("testing_recursion %s\n",initial_recursion[zuper]);
	}
	...
	while (1){//creating schedule
		...
		if(courses_taken != 0){
			printf("Year_number = %d Quarter_number = %d Courses taken: ", year_number, quarter_number);
			int current_class = 0;
